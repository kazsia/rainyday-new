...
export async function getDashboardStats(range?: { from: Date; to: Date }): Promise<DashboardData> {
    ...
    const { data: revCurrent } = await supabase.from("payments")
        .select("amount")
        .eq("status", "completed")
        .gte("created_at", from.toISOString())
        .lte("created_at", to.toISOString())

    const { data: revPrev } = await supabase.from("payments")
        .select("amount")
        .eq("status", "completed")
    ...
    const { count: ordCurrent } = await supabase.from("orders")
        .select("*", { count: 'exact', head: true })
        .gte("created_at", from.toISOString())
        .lte("created_at", to.toISOString())

    const { count: ordPrev } = await supabase.from("orders")
        .select("*", { count: 'exact', head: true })
        .gte("created_at", prevFrom.toISOString())
        .lt("created_at", prevTo.toISOString())
    ...
    const { count: custCurrent } = await supabase.from("profiles")
        .select("*", { count: 'exact', head: true })
        .gte("created_at", from.toISOString())
        .lte("created_at", to.toISOString())

    const { count: custPrev } = await supabase.from("profiles")
        .select("*", { count: 'exact', head: true })
        .gte("created_at", prevFrom.toISOString())
        .lt("created_at", prevTo.toISOString())
