...
// Link import removed from here (it is at top)
export const LiquidButton = ({ text = "Get Started", onClick, className, href }: LiquidButtonProps) => {
    const [isHovered, setIsHovered] = useState(false);

    return (
        <div className={cn("relative inline-block w-36 h-[2.7em] mx-auto group dark:bg-black bg-white dark:border-white border-black border-2 rounded-lg cursor-pointer overflow-hidden", className)}>
            ...
            {href ? (
                <Link
                    href={href}
                    className="absolute inset-0 rounded-lg bg-transparent cursor-pointer w-full h-full z-10 flex items-center justify-center pointer-events-auto"
                    aria-label={text}
                    onMouseEnter={() => setIsHovered(true)}
                    onMouseLeave={() => setIsHovered(false)}
                >
                    <span className="flex items-center justify-center px-4 gap-2 rounded-lg group-hover:text-yellow-400 text-white text-xl font-semibold tracking-wide whitespace-nowrap w-full h-full">
                        {text}
                    </span>
                </Link>
            ) : (
                <button
                    className="absolute inset-0 rounded-lg bg-transparent cursor-pointer w-full h-full z-10 pointer-events-auto"
                    aria-label={text}
                    type="button"
                    onClick={onClick}
                    onMouseEnter={() => setIsHovered(true)}
