-- Add delivery settings to products table

alter table public.products
add column if not exists deliverable_selection_method text not null default 'last',
add column if not exists disabled_payment_methods jsonb not null default '[]'::jsonb;

-- Add check constraint for deliverable_selection_method
alter table public.products
add constraint check_deliverable_selection_method
check (deliverable_selection_method in ('last', 'first', 'random'));

-- Update claim_stock function to support selection methods
